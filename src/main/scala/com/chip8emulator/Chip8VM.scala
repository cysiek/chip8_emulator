...
//      val rightHandSideOfComparison: Byte = if (instruction.parts(0) == 5) registers(instruction.operandY.get) else bitOperations.pack2ValuesToByte(instruction.parts(2), instruction.parts(3))
      val rightHandSideOfComparison: Byte = if (List(5, 9) contains instruction.parts(0)) registers(instruction.operandY.get) else bitOperations.pack2ValuesToByte(instruction.parts(2), instruction.parts(3))
      ...
      if (executeJump) {
//        pcRegister = (pcRegister + 2).toShort
        pcRegister = (pcRegister + 4).toShort
      }
      !executeJump
    } else
